...
#| echo: false
#| output: false
i = rand(findall(p -> model.τ - 0.32 <= p <= model.τ - 0.30, predict(model; threshold=false)))
cnd = X[:,i]
...
#| echo: false
#| output: false
temp = cnd[model.v[1]] .+ LinRange(-70, 70, 200)
...
#| echo: false
#| output: false
proposals = [counterfactual(model, cnd, model.τ + 0.015, 500.0; threshold=false) for _ in 1:50]
```
...
for i in eachindex(proposals)
    ...
    scatter!(ax, [xc[model.v[1]]], [xc[model.v[2]]], strokecolor=ccol, strokewidth=2, color=bcol, markersize=12)
end
vlines!(ax, [cnd[model.v[1]]], color=bkcol.generic, linestyle=:dash)

...
#| echo: false
#| output: false
i = rand(findall(p -> model.τ - 0.22 <= p <= model.τ - 0.20, predict(model; threshold=false)))
cnd = X[:,i]
...
scatter!(ax, [cnd[6]], [pscore], color=bkcol.generic, markersize=12)
...
#| echo: false
#| output: false
proposals = [counterfactual(model, cnd, model.τ + 0.015, 250.0; threshold=false) for _ in 1:100]
```
...
for i in eachindex(proposals)
    ...
    scatter!(ax, [xc[6]], [xc[3]], strokecolor=ccol, strokewidth=2, color=bcol, markersize=12)
end
vlines!(ax, [cnd[6]], color=bkcol.generic, linestyle=:dash)
hlines!(ax, [cnd[3]], color=bkcol.generic, linestyle=:dash)
scatter!(ax, [cnd[6]], [cnd[3]], color=bkcol.generic, markersize=12)
...
L = [loss(model, cnd, xp, model.τ + 0.015, 250.0; threshold=false) for xp in proposals]

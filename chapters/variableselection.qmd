...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["", "PCA", length(predictors(mod)), mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
hl_maxval = MarkdownHighlighter(
           (data, i, j) -> (j == 4) && (D[i,4] == maximum(Float64.(D[:,4]))),
           MarkdownDecoration(bold = true)
       );
...
pretty_table(
    D;
    backend = Val(:markdown),
    header = ["Variable selection", "Transformation", "Nb. variables", "MCC (val.)", "MCC (train)"],
    alignment = [:l, :l, :l, :l, :l],
    formatters = ft_printf("%5.3f", [4,5]),
    highlighters = hl_maxval
)
```
...
ax = Axis(f[1,1], xticks = (1:length(Vm), ["BIO$(v)" for v in model.v]))

stem!(ax, Vm ./ sum(Vm), strokecolor=bkcol.generic, color=:white, strokewidth=1, trunkcolor=:transparent, stemcolor=bkcol.nodata, markersize=18)

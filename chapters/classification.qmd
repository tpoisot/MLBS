...
#| echo: false
#| fig-cap: Overview of the scores for the four components of the confusion matrix (false/true positive/negative rates), as well as two summary measures, the True-Skill statistic (Youden's J) and Matthew's correlation coefficient.
fmat = [(tpr, "True positives") (fpr, "False positives") (trueskill, "TSS"); (tnr, "True negatives") (fnr, "False negatives") (mcc, "MCC")]
f = Figure()
...
|------------------|------------------|------------------|------------------|
| True positive rate                | `{julia} round(mean(tpr.(Ct)); digits=2)`       | `{julia} round(mean(tpr.(Cv)); digits=2)`       | `{julia} round(tpr(Cnoskill); digits=2)`       |
| False positive rate               | `{julia} round(mean(fpr.(Ct)); digits=2)`       | `{julia} round(mean(fpr.(Cv)); digits=2)`       | `{julia} round(fpr(Cnoskill); digits=2)`       |
| True negative rate                | `{julia} round(mean(tnr.(Ct)); digits=2)`       | `{julia} round(mean(tnr.(Cv)); digits=2)`       | `{julia} round(tnr(Cnoskill); digits=2)`       |
| False negative rate               | `{julia} round(mean(fnr.(Ct)); digits=2)`       | `{julia} round(mean(fnr.(Cv)); digits=2)`       | `{julia} round(fnr(Cnoskill); digits=2)`       |
| True-Skill statistic              | `{julia} round(mean(trueskill.(Ct)); digits=2)` | `{julia} round(mean(trueskill.(Cv)); digits=2)` | `{julia} round(trueskill(Cnoskill); digits=2)` |
| Matthew's correlation coefficient | `{julia} round(mean(mcc.(Ct)); digits=2)`       | `{julia} round(mean(mcc.(Cv)); digits=2)`       | `{julia} round(mcc(Cnoskill); digits=2)`       |
...
#| echo: false
#| fig-cap: Prediction mapped in ...
f = Figure(; resolution=(500, 500))
ax = GeoAxis(f[1,1], xlabel="Longitude", ylabel = "Latitude", lonlims=extrema(longitudes(pr)), latlims=extrema(latitudes(pr)), dest = "+proj=moll")
surface!(ax, pr.>0.5, shading=false, colormap=vibrant[1:2], interpolate=false)
ax.xticklabelpad = 30
ax.yticklabelpad = 40
current_figure()
```
...
```{julia}
#| echo: false
samplemodels = [naivebayes(trainlabels, trainfeatures[b,:]) for b in bootstrap(trainlabels, trainfeatures)]
...
```{julia}
#| label: fig-classification-bootstrap
#| echo: false
#| fig-cap: After re-training 50 version of the model over randomly bootstrapped datasets, we measure the proportion of the bootstrap models that agree with the original prediction at each pixel. Higher values correspond to a more certain prediction, therefore to pixels where we can be more confident in the model output. In subsequent chapters, we will introduce additional tools to reason about the uncertainty in the model.
f = Figure(; resolution=(500, 500))
ax = GeoAxis(f[1,1], xlabel="Longitude", ylabel = "Latitude", lonlims=extrema(longitudes(pr)), latlims=extrema(latitudes(pr)), dest = "+proj=moll")
sf = surface!(ax, un, shading=false, colormap=cgrad(iridescent), interpolate=false)
ax.xticklabelpad = 30
ax.yticklabelpad = 40
Colorbar(f[1,2],sf)
current_figure()

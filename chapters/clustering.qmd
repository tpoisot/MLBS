...
## The problem: classifying pixels from an image {#sec-clustering-data}
...
#| echo: false
#| output: false
R = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B4.tif"), SimpleSDMResponse)
G = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B3.tif"), SimpleSDMResponse)
B = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B2.tif"), SimpleSDMResponse)
NIR = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B5.tif"), SimpleSDMResponse)
SWIR1 = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B6.tif"), SimpleSDMResponse)
SWIR2 = SpeciesDistributionToolkit._read_geotiff(joinpath(_data_path, "earthengine", "LandSat.SR_B7.tif"), SimpleSDMResponse)
```

In fact, we are going to use an image with many dimensions: the data in this chapter are coming from a Landsat 8 scene [@vermote2016], for which we have access to many different bands.
...
| 6, 7 | Short wavelength IR (SWIR 1 and 2) | Good at differentiating wet earth and dry earth |
...
ax = Makie.Axis(fig[1,1]; xlabel="NBR", ylabel="NDWI", aspect=DataAspect())
hexbin!(ax, NBR, NDWI, colormap=cgrad(bkcol.seq), bins=50)
tightlimits!(ax)
...
## The theory behind *k*-means clustering {#sec-kmeans-theory}
...
digraph G {
    ...
    rndcnt[label="Random centroids",shape=plaintext]
    cntr[label="Update centroids",shape=underline]
    opt[shape=underline, label="Optimum reached?"]
    clstr[label="Optimal centroids",shape=rect]

    optyes[shape="plaintext" fontcolor="darkgreen" label="✓"];
    optno[shape="plaintext" fontcolor="darkred" label="✗"];
...
which is the value of $j$ that minimizes the $L^2$ norm ($\|\cdot\|_2$, the Euclidean distance\index{Distances!Euclidean}) between the instance and the centroid; $\text{argmin}_j$ is the function returning the value of $j$ that minimizes its argument. For example, $\text{argmin}(0.2,0.8,0.0)$ is $3$, as the third argument is the smallest. There exists an $\text{argmax}$ function, which works in the same way.

### Optimizing the centroids

Of course, what we really care about is the assignment of *all* instances to the classes. For this reason, the configuration (the disposition of the centroids) that solves our specific problem is the one that leads to the lowest possible variance\index{Variance} within the clusters. As it turns out, it is not that difficult to go from @eq-clustering-onepoint to a solution for the entire problem: we simply have to sum over all points!
...
::: column-margin
Repeating a step multiple times in a row is called an iterative process, and we will see a *lot* of them. @sec-gradientdescent relies almost entirely on iteration, in fact.
:::

Once we have applied @eq-clustering-centroid-update to all classes, there is a good chance that we have moved the centroids in a way that moved them away from some of the points, and closer to others: the membership of the instances has likely changed. Therefore, we need to re-start the process again, in an iterative way.\index{Iteration}

But until when?

Finding the optimal solution\index{Optimality} for a set of points is an NP-hard problem [@aloise2009], which means that we will need to rely on a little bit of luck, or a whole lot of time. The simplest way to deal with iterative processes is to let them run for a long time, as after a little while they should converge onto an optimum (here, a set of centroids for which the variance is as good as it gets), and hope that this optimum is *global* and not *local*.

A global optimum\index{Optimality!global} is easy to define: it is the state of the solution that gives the best possible result. For this specific problem, a global optimum means that there are no other combinations of centroids that give a lower variance. A local optimum is a little bit more subtle: it means that we have found a combination of centroids that we cannot improve without first making the variance worse. Because the algorithm as we have introduced it in the previous sections is *greedy*, in that it makes the moves that give the best short-term improvement, it will not provide a solution that temporarily makes the variance higher, and therefore is susceptible to being trapped in a local optimum\index{Optimality!local}.
...
heatmap!(ax, C; colormap=bkcol.cat)
hidedecorations!(ax)
current_figure()
```
...
We will now change the values of $k$ and use the @davies1979 measure introduced in @sec-clustering-optimality to identify the optimal value of $k$. The results are presented in @fig-kmeans-tuning. Note that we only explore $k \in [3, 10]$. More than 10 categories is probably not very actionable\index{Actionable models}, and therefore we can make the decision to only look at this range of parameters. Sometimes (always!) the best solution is the one that gets your job done.

Note that we will play a little trick on our data here. We know from @fig-kmeans-initial-landscape that with $k=3$, we can do a good job at removing water pixels, which seem to cover about a third of the image. If we are interested in land-based questions, this is quite a lot of information we do not really care about. In order to make the clustering more relevant to our interests, we will therefore extract the pixels that are *not* classified as water before doing the clustering. This *will* miss a few relevant pixels, but for now we can consider this an acceptable trade-off in order to have a better classification of land pixels.

```{julia}
#| label: mask-water
#| echo: false
#| output: false
msk = convert(Bool, similar(NDVI))
msk.grid[findall(isequal(C[1]), permutedims(C))] .= true # We know the first pixel in C is water
replace!(msk, true => nothing)
replace!(msk, false => true)

X = zeros(Float32, (4, length(msk)))
X[1,:] .= values(SpeciesDistributionToolkit.mask(msk, NDVI))
X[2,:] .= values(SpeciesDistributionToolkit.mask(msk, NDWI))
X[3,:] .= values(SpeciesDistributionToolkit.mask(msk, NDMI))
X[4,:] .= values(SpeciesDistributionToolkit.mask(msk, NBR))
```

What would make such a classification better? Recall from the @sec-kmeans-theory that the assignment of instances to classes is done based on the distance to the centroid, which it itself the average of the features of the members of each class. By removing the pixels that are not relevant to our question (such as using water pixels to classify the image using data relevant to vegetation), we are taking steps to ensure that the space in which the centroids can exist is more rigorously defined for the question we are interested in.\index{Data preparation}
...
ax1 = CairoMakie.Axis(gl[1,1]; xlabel="Number of clusters", ylabel="Davies-Bouldin index", xticks=3:maximum(k_trial))
scatterlines!(ax1, k_trial, score, color=:black)
errorbars!(ax1, k_trial, score, .5vscore, .5vscore, whiskerwidth = 10, color=:black)

ax2 = CairoMakie.Axis(gl[2,1]; xlabel="Number of clusters", ylabel="Total cost", xticks=3:maximum(k_trial))
scatterlines!(ax2, k_trial, costs, color=:black)
errorbars!(ax2, k_trial, costs, .5vcosts, .5vcosts, whiskerwidth = 10, color=:black)
...
There are two interesting things in @fig-kmeans-tuning. First, note that for $k=\{3,4,5\}$, there is almost no dispersal: all of the assignments have the exact same score, which is unlikely to happen except if the assignments are the same every time! This is a good sign, and, anecdotally, something that might suggest a really information separation of the points. Second, $k = 3$ has by far the lowest Davies-Bouldin index of all values we tried, and is therefore strongly suggestive of an optimal hyper-parameter. Because we used the information in @fig-kmeans-initial-landscape to remove the water pixels, we can then probably move forward with this value.
...
clusters = kmeans(X, k)
C = zeros(size(NDVI))
C[findall(!isnothing, msk.grid)] .= clusters.assignments
C[findall(iszero, C)] .= NaN
```

### Clustering with optimal number of classes

The clustering of pixels using $k = 3$ is presented in @fig-kmeans-optimal-landscape. Unsurprisingly, *k*-means separated the open water pixels, the dense urban areas, as well as the more forested/green areas. Now is a good idea to start thinking about what is representative of these clusters: one is associated with very high NDWI value (these are the water pixels), and two classes have both high NDVI and high NDMI (suggesting different categories of vegetation).
...
heatmap!(ax_clust, permutedims(C); colormap=bkcol.cat)
...
:::
  ...
  [Karthik Ram's Wes Anderson palettes]: https://github.com/karthik/wesanderson
  [David Lawrence Miller's Beyoncé palettes]: https://github.com/dill/beyonce
...
scatter!(ax, values(SpeciesDistributionToolkit.mask(msk, NDVI)), values(SpeciesDistributionToolkit.mask(msk, NDMI)), color=clusters.assignments, colormap=bkcol.cat, markersize=2)
tightlimits!(ax)
current_figure()
```
...
But maybe we espouse a more ambitious definition of a model, and we are specifically looking for a model that can makes prediction on data that we had not previously used. In that case, *k*-means still qualifies. Imagine, for a moment, a new pixel. This is a little unrealistic as, unless some unforeseen geological events took place, all of the pixels we could describe today for this dataset already existed in 2017. But imagine we changed the bounding box of the scene: we now have spectral data for a few thousand pixels we had not previously observed.

We do not need to run *k*-means again to classify these pixels. Because *k*-means is trained when we have established the number and position of the centroids, we can now simply figure out which cluster the new point belongs to, which is done using @eq-clustering-onepoint.

Both "what is a model" and "what is a new datapoint" are important questions to think about, and there are often edge cases to the answers we can bring. They will come back often in this book, notably in @sec-crossvalidation. It is always a good idea, at the end of a chapter, to stop and think for a moment about what our model *really* is. What problems can it can solve? What would it would mean to use this model on new data? In doing so, you will naturally start thinking about the place of each model in the broader problem of "how do I make my data make sense?".

## Conclusion

In this chapter, we have used the *k*-means algorithm to create groups in a large dataset that had no labels, *i.e.* the points were not initially assigned to a class. By picking the features we wanted to cluster the different points, we were able to highlight specific aspects of the landscape. In @sec-gradientdescent, we will start adding labels to our data, and shift our attention from classification to regression problems.
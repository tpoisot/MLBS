...
: Overview of the bands in a Landsat 8 scene. The data from this chapter were downloaded from [LandsatLook].
...
::: column-margin
The correct distance measure to use depends on what is appropriate for the data!
:::
...
heatmap!(ax, C; colormap=light[1:3])
hidedecorations!(ax)
current_figure()
```
...
msk.grid[findall(isone, permutedims(C))] .= true
...
Both "what is a model" and "what is a new datapoint" are important questions to think about; they will come back often in this book, notably in @sec-crossvalidation. It is always idea, at the end of a chapter, to stop and think for a moment about what our model is, what problems it can solve, and what it would mean to use it on new data. In doing so, you will naturally start thinking about the place of each model in the broader problem of "how do I make data make sense?".

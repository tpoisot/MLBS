...
#| echo: false
#| output: false
samplemodels = [train!(deepcopy(model), y[b[1]], X[:,b[1]]; classify=false) for b in bootstrap(y, X; n=500)]
un = convert(Float64, similar(first(bio)))
pr = convert(Float64, similar(first(bio)))
Threads.@threads for k in keys(pr)
    x = [b[k] for b in bio]
    pr[k] = predict(model, x)
    un[k] = mean([predict(m, x)==predict(model, x) for m in samplemodels])
end
```

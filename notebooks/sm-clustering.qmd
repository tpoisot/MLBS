...
#| fig-cap: The Landsat 9 data are combined into the "Natural Color" image, in which the red, green, and blue bands are mapped to their respective channels (left). The two other composites are the composite for vegetation analysis (middle) and the false-color infrared (bottom). Note that the true-color composite is slightly distored compared to the colors of the landscape we expect; this is because natural colors are difficult to reproduce accurately.
...
ax_cir = Makie.Axis(fig[3,1], aspect=DataAspect())
...
ax_clust = Makie.Axis(fig[1,1], aspect=DataAspect())
heatmap!(ax_clust, C; colormap=Makie.wong_colors()[1:3])
...
filter open water pixels

```{julia}
landmass = findall(X[2,:] .< 0.0)
```
...
Suppressor.@suppress_err begin
    Threads.@threads for j in axes(k_trial, 1)
        ...
        for rep in axes(sc, 1)
            cl = kmeans(X[:,landmass], k)
            sc[rep] = DaviesBouldin(X[:,landmass], cl)
            cs[rep] = cl.totalcost
        end
        score[j] = mean(sc)
        costs[j] = mean(cs)
...
k = 4 #k_trial[imax]
clusters = kmeans(X[:,landmass], k)
@info DaviesBouldin(X[:,landmass], clusters)
```
...
C = fill(NaN, size(ndvi))
C[landmass] .= clusters.assignments
fig = Figure(resolution=(500, 150))
ax_clust = Makie.Axis(fig[1,1], aspect=DataAspect())
heatmap!(ax_clust, permutedims(C); colormap=Makie.wong_colors()[1:k])
hidedecorations!(ax_clust)
current_figure()
```
...
fig = Figure(resolution=(500,300))
ax = Makie.Axis(fig[1,1]; xlabel="Vegetation", ylabel="Moisture")
scatter!(ax, vec(ndvi), vec(ndmi), color=clusters.assignments, colormap=Makie.wong_colors()[1:k])
tightlimits!(ax)
current_figure()
```

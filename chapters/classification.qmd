...
axtemp = Axis(gl[1,1])
c = light[2]
density!(axtemp, filter(r -> r.presence, occdata).BIO1, color=(c, 0.3), strokecolor=c, strokewidth=1, normalization=:pdf)
c = light[3]
density!(axtemp, filter(r -> !r.presence, occdata).BIO1, color=(c, 0.3), strokecolor=c, strokewidth=1, normalization=:pdf)
...
xp = filter(r -> r.presence, occdata).BIO1
xn = filter(r -> !r.presence, occdata).BIO1
...
|-----|--------|---------|---------|
| True positive rate | `{julia} round(mean(tpr.(conftrain)); digits=2)` | `{julia} round(mean(tpr.(confvalid)); digits=2)` | `{julia} round(tpr(Cnoskill); digits=2)` | 
| False positive rate | `{julia} round(mean(fpr.(conftrain)); digits=2)` | `{julia} round(mean(fpr.(confvalid)); digits=2)` | `{julia} round(fpr(Cnoskill); digits=2)` | 
| True negative rate | `{julia} round(mean(tnr.(conftrain)); digits=2)` | `{julia} round(mean(tnr.(confvalid)); digits=2)` | `{julia} round(tnr(Cnoskill); digits=2)` | 
| False negative rate | `{julia} round(mean(fnr.(conftrain)); digits=2)` | `{julia} round(mean(fnr.(confvalid)); digits=2)` | `{julia} round(fnr(Cnoskill); digits=2)` | 
| True-Skill statistic | `{julia} round(mean(trueskill.(conftrain)); digits=2)` | `{julia} round(mean(trueskill.(confvalid)); digits=2)` | `{julia} round(trueskill(Cnoskill); digits=2)` | 
| Matthew's correlation coefficient | `{julia} round(mean(mcc.(conftrain)); digits=2)` | `{julia} round(mean(mcc.(confvalid)); digits=2)` | `{julia} round(mcc(Cnoskill); digits=2)` | 

...
include(joinpath(_code_path, "minisdm/confusionmatrix.jl"))
include(joinpath(_code_path, "minisdm/pipelines.jl"))
include(joinpath(_code_path, "minisdm/nbc.jl"))
include(joinpath(_code_path, "minisdm/mocks.jl"))
include(joinpath(_code_path, "minisdm/crossvalidation.jl"))
include(joinpath(_code_path, "minisdm/bootstrap.jl"))
include(joinpath(_code_path, "minisdm/show.jl"))
```
...
JLD2.jldsave(trainingpath; y=y, X=X)
X = X[[1,12],:]
```
...
axprec = Axis(gl[2,2])
c = vibrant[2]
density!(axprec, filter(r -> r.presence, occdata).BIO12, color=(c, 0.3), strokecolor=c, strokewidth=1, normalization=:pdf, direction=:y)
c = vibrant[1]
density!(axprec, filter(r -> !r.presence, occdata).BIO12, color=(c, 0.3), strokecolor=c, strokewidth=1, normalization=:pdf, direction=:y)
...
::: column-margin
Whenever possible, avoid using accuracy except to communicate the skill of the model is easy to understand terms.
:::
...
In order to see how good our model really is, we will also compare its performances to the no-skill classifier. This is almost never a difficult classifier to outperform, but this nevertheless provides a good indication of whether our model works *at all*. In a later chapter, we will introduce a slightly more domain-specific model to provide a baseline that would look like an actual model we would like to out-perform.
...
Cv, Ct = crossvalidate(model, y, X, folds)
```
...
Threads.@threads for k in keys(pr)
    x = [b[k] for b in bio]
    pr[k] = predict(model, x; classify=true)
end
```
...
ax0 = Axis(gl[1,1], xlabel="Longitude", ylabel = "Latitude", aspect = DataAspect())
ax1 = Axis(gl[1,2], xlabel="Longitude", ylabel = "Latitude", aspect = DataAspect())
ax2 = Axis(gl[1,3], xlabel="Longitude", ylabel = "Latitude", aspect = DataAspect())

heatmap!(ax1, pr, colormap=vibrant[1:2])

heatmap!(ax0, pr, colormap=[light[1]])
scatter!(ax0, coordinates.longitude[trn], coordinates.latitude[trn], color=coordinates.presence[trn], colormap=[:black, vibrant[2]], markersize=4)

heatmap!(ax2, pr, colormap=[light[1]])
prd = predict(model, X; classify=true)
...
scatter!(ax2, coordinates.longitude[trn][TP], coordinates.latitude[trn][TP], markersize=5, color=vibrant[2])
scatter!(ax2, coordinates.longitude[trn][FP], coordinates.latitude[trn][FP], markersize=6, marker=:cross, color=:black)

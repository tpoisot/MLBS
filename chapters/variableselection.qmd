...
::: column-margin
In @ec-tuning, we will revisit the question of how the MCC is "better", and spend more time evaluating alternatives. For now, we can safely *assume* that MCC is the best.
:::
...
heatmap!(ax1, rangediff(inter_range, old_range); colormap = Reverse(cgrad(nightfall, 3, categorical=true)))
heatmap!(ax2, rangediff(final_range, inter_range); colormap = Reverse(cgrad(nightfall, 3, categorical=true)))

cleg = [PolyElement(color = c, strokecolor = :transparent) for c in cgrad(nightfall, 3, categorical=true)]
clab = ["Gain", "No change", "Loss"]
Legend(fig[2,1:2], cleg, clab; tellheight=false, tellwidth=false, halign=:center, valign=:top, margin=(5, 5, 5, 5), nbanks=3, framevisible=false)

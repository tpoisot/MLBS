...
colors = [x<0 ? light[3] : light[2] for x in S]

arrows!(ax, starts, elev, S, ydir, color=colors, align=:tailhead)
scatter!(ax, starts, elev, color=colors, markersize=8)
...
xmin, xmax = maximum(abs.(extrema(S[:,vord[1:4]]))) .* (-1, 1)
...
cleg = [PolyElement(color = c, strokecolor = :transparent) for c in vibrant[1:2]]
clab = ["Absent", "Present"]
Legend(f[1:3,2], cleg, clab; tellheight=false, tellwidth=false, halign=:left, valign=:top, margin=(10, 10, 10, 10), framevisible=false)
...
Threads.@threads for k in keys(V[1])
    x = [b[k] for b in bio]
    for i in axes(v, 1)
        V[i][k] = shapley(model, X, x, i; M=250)
    end
end
```

...
fig = Figure(; size=(7, 5).*(600/6))
ax1 = Axis(fig[1,1]; xlabel="Holdout proportion", ylabel="MCC")

μ = vec(mean(valm; dims=2))
σ = vec(std(valm, dims=2))
c = 1.96 .* σ ./ sqrt(_repls)
band!(ax1, _prop, μ-c, μ+c, color = (bkcol.cv.validation, 0.4))
scatterlines!(ax1, _prop, μ, color = bkcol.cv.validation, markersize=4, linewidth=1)

μ = vec(mean(trnm; dims=2))
σ = vec(std(trnm, dims=2))
c = 1.96 .* σ ./ sqrt(_repls)
band!(ax1, _prop, μ-c, μ+c, color = (bkcol.cv.training, 0.4))
scatterlines!(ax1, _prop, μ, color = bkcol.cv.training, markersize=4, linewidth=1)
...
series!(ax, T, mcc.(CT), solid_color=bkcol.cv.training)
lines!(ax, T, vec(mean(mcc.(CT); dims=1)), color=bkcol.cv.validation, linewidth=2, linestyle=:dash)
scatter!(ax, model.τ, maximum(vec(mean(mcc.(CT); dims=1))), strokecolor=bkcol.cv.validation, color=:white, strokewidth=2, markersize=20)
xlims!(ax, (0., 1.))
ylims!(ax, (0., 1.))
current_figure()
```
...
poly!(rg, Point2f[(0, -0.05), (model.τ, -0.05), (model.τ, 0.6), (0, 0.6)], color=bkcol.sdm.absentbg)
poly!(rg, Point2f[(model.τ, -0.05), (1, -0.05), (1, 0.6), (model.τ, 0.6)], color=bkcol.sdm.presentbg)
...
errorbars!(ax1, T, vec(mean(ppv.(CT); dims=1)), vec(std(ppv.(CT); dims=1)), color=bkcol.nodata)
lines!(ax1, T, vec(mean(ppv.(CT); dims=1)), color=bkcol.generic, linewidth=2, linestyle=:solid)

errorbars!(ax2, T, vec(mean(npv.(CT); dims=1)), vec(std(npv.(CT); dims=1)), color=bkcol.nodata)
lines!(ax2, T, vec(mean(npv.(CT); dims=1)), color=bkcol.generic, linewidth=2, linestyle=:solid)
...
for i in axes(CT, 1)
    lines!(roc, fpr.(CT[i,:]), tpr.(CT[i,:]), color=(bkcol.generic, 0.6))
    lines!(pr, tpr.(CT[i,:]), ppv.(CT[i,:]), color=(bkcol.generic, 0.6))
end
...
for (i, P) in enumerate(Pi)
    ...
    @info mcc.(crossvalidate(Pm[i], folds; threshold=true)[1]) |> mean
    @info Pm[i].classifier.prior
end
```
...
#| fig-scap: Tuning of the NBC prior probability (see @eq-nbc-onevar) that a location is favorable to the presence of the species. As in @fig-tuning-threshold, the model was cross-validation on a number of priors increasing from 0 (presences will never be predicted) to 1 (presences will always be predicted). The vertical dashed line is the class balance in the dataset: the optimal prior for this model is informed by the relative proportion of presences in the training data.
#| fig-cap: The 
...
ax = Axis(f[1,1], xlabel="Prior", ylabel="MCC", aspect=1)

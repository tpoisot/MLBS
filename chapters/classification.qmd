...
A very important feature of the relationship between the features and the classes is that, broadly speaking, classification is much easier when the classes are separable. Separability (often linear separability) is achieved when, if looking at some projection of the data on two dimensions, you can draw a line that separates the classes (a point in a single dimension, a plane in three dimension, and so on and so forth). For reasons that will become clear in @sec-predictors-curse, simply adding more predictors is not the right thing to do.\index{Separability}

In @fig-classification-separability, we can see the temperature (in degrees) for locations with recorded presences of Corsican nuthatches, and for locations with assumed absences. These two classes are not quite linearly separable alongside this single dimension (maybe there is a different projection of the data that would change this; we will explore several such projections in @sec-predictors), but there are still some values at which our best guess for a class changes. For example, at a location with a temperature colder than 10°C, presences are far more likely. For a location with a temperature warmer than 15°C, absences become overwhelmingly more likely. The locations with a temperature between 10°C and 15°C can go either way (and this ambiguity is precisely why we rely on species distribution models).

```{julia}
#| label: fig-classification-separability
#| echo: false
#| fig-scap: Separability of presences and absences in the two-variables classifier.
#| fig-cap: This figures show the separability of the presences and pseudo-absences on the temperature and precipitation dimensions.
f = Figure(; size=(500, 500))

gl = f[1,1] = GridLayout()

axtemp = Axis(gl[1,1])
density!(axtemp, X[1,findall(y)], color=bkcol.sdm.presentbg, strokecolor=bkcol.sdm.present, strokewidth=1)
density!(axtemp, X[1,findall(!, y)], color=bkcol.sdm.absentbg, strokecolor=bkcol.sdm.absent, strokewidth=1)

axprec = Axis(gl[2,2])
density!(axprec, X[12,findall(y)], color=bkcol.sdm.presentbg, strokecolor=bkcol.sdm.present, strokewidth=1, direction=:y)
density!(axprec, X[12,findall(!, y)], color=bkcol.sdm.absentbg, strokecolor=bkcol.sdm.absent, strokewidth=1, direction=:y)

axboth = Axis(gl[2,1], xlabel="Mean air temperature (°C)", ylabel = "Annual precipitation (kg m⁻²)")
scatter!(axboth, X[1,findall(!, y)], X[12,findall(!, y)], color=bkcol.sdm.absent, markersize=4, label="Pseudo-absence")
scatter!(axboth, X[1,findall(y)], X[12,findall(y)], color=bkcol.sdm.present, markersize=4, label="Presence")
...
Evaluating the performance of a classifier (a classifier is a model that performs classification) is usually done by looking at its confusion table, which is a contingency table of the form\index{Confusion table}
...
The four components of the confusion table are the true positives (TP; $\hat +$ is the correct prediction of $+$), the true negatives (TN; $\hat -$ is th ecorrect prediction of $-$), the false positives (FP; $\hat -$ is the incorrect prediction of $+$), and the false negatives (FN; $\hat -$ is the incorrect prediction of $-$). Quite intuitively, we would like our classifier to return mostly elements in TP and TN: a good classifier has most element on the diagonal, and off-diagonal elements as close to zero as possible (the proportion of predictions on the diagonal is called the accuracy, and we will spend @sec-classification-accuracy discussing why it is not such a great measure).
...
Balance refers to the proportion of the positive class. Whenever this balance is not equal to 1/2 (there are as many positives as negative cases), we are performing *imbalanced* classification, which comes with additional challenges; few ecological problems are balanced.\index{Class imbalance}
...
An alternative to the no-skill classifier is the coin-flip classifier, in which classes have their correct prevalence $b$, but the model picks at random with probability $1/2$ within these classes. This differs from the no-skill classifier by adopting a different random chance of picking a class while still respecting the prevalence of the positive class.\index{Confusion table!Coin-flip classifier}
...
This classifier has the confusion matrix\index{Confusion table!Constant classifier}
...
The accuracy of this classifier would be $0.95$, which seems extremely high! This is because prevalence is extremely low, and so most of the predictions are about the negative class: the model is *on average* really good, but is completely missing the point when it comes to making interesting predictions.\index{Model performance!Paradox of accuracy}
...
In this chapter, we will rely on Monte-Carlo cross validation (MCCV; see @sec-crossvalidation-montecarlo), using 100 replicates. In the following chapters, we will revert to using k-folds cross-validation, but using MCCV here is a good enough starting point.\index{Cross-validation!Monte-Carlo}

In order to see how good our model really is, we will also compare its performances to various null classifiers. There are almost never difficult classifiers to outperform, but this nevertheless provides a good indication of whether our model works *at all*. In @sec-squint, we will introduce a slightly more domain-specific model to provide a baseline that would look like an actual model we would like to out-perform (but mostly to make the general point that any problem can be approached like a machine learning problem).\index{Confusion table!No-skill classifier}
...
model = SDM(RawData(), NaiveBayes(), 0.5, X, y, [1, 12])
Cv, Ct = crossvalidate(model, folds; threshold=false)
```

### Performance evaluation of the model

In order to get a sense of the performance of our model, we will need to decide on a performance measure. This is an important step, as we will use the average value of this measure on the validation data to decide on the best model *before* reporting the expected performance. If we pick a measure that is biased, we will therefore use a model that is biased. Following @chicco2020 and @Jurman2012, we will use the Matthew's Correlation Coefficient (MCC) as the "main" measure to evaluate the performance of a model (we will return to other alternative measures in @sec-tuning, and eventually explain why MCC is the most appropriate for classification evaluation).\index{Confusion table!Matthew's Correlation Coefficient}
...
fmat = [(mcc, "Matthew's corr."), (ppv, "Pos. pred. val."), (npv, "Neg. pred. val."), (tpr, "True pos. rate"), (tpr, "True neg. rate"), (balancedaccuracy, "Bal. acc."), (accuracy, "Accuracy")]
fig = Figure(; size=(600, 300))

ax = Axis(fig[1,1], xticks = (1:length(fmat), last.(fmat)), xticklabelrotation=45.0)

for i in axes(fmat, 1)
    f, fn = fmat[i]
    errorbars!(ax, [i-0.05], [f(Cv)], [ci(Cv, f)], whiskerwidth=6, color=bkcol.cv.validation, label="Validation")
    scatter!(ax, [i-0.05], [f(Cv)], color=bkcol.cv.validation, label="Validation")
    errorbars!(ax, [i+0.05], [f(Ct)], [ci(Ct, f)], whiskerwidth=6, color=bkcol.cv.training, label="Training")
    scatter!(ax, [i+0.05], [f(Ct)], color=bkcol.cv.training, label="Training")
end

fig[1,2] = Legend(fig, ax, merge=true, unique=true, nbanks=1)
...
air = LinRange(extrema(X[1,:])..., 250)
prc = LinRange(extrema(X[12,:])..., 250)
...
axboth = Axis(gl[2,1], xlabel="Mean air temperature (°C)", ylabel = "Annual precipitation (kg m⁻²)")
heatmap!(axboth, air, prc, SRF, colormap=[bkcol.sdm.absent, bkcol.sdm.present], alpha=0.3)
scatter!(axboth, X[1,findall(!, y)], X[12,findall(!, y)], color=bkcol.sdm.absent, markersize=4, label="Pseudo-absence")
scatter!(axboth, X[1,findall(y)], X[12,findall(y)], color=bkcol.sdm.present, markersize=4, label="Presence")
...
The line that separates the two classes is usually referred to as the "decision boundary" of the classifier: crossing this line by moving in the space of features will lead the model to predict another class at the output. In this instance, as a consequence of the choice of models and of the distribution of presence and absences in the environmental space, the decision boundary is not linear.\index{Decision boundary}
...
heatmap!(ax3, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])

heatmap!(ax0, pr; colormap = [bkcol.nodata])
scatter!(
    ax0,
    rotl90(coordinates);
    color = y,
    colormap = [bkcol.sdm.absent, bkcol.sdm.present],
    markersize = 2,
)
...
heatmap!(ax1, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(ax1, rotl90(coordinates[TP,:]); markersize = 2, color = bkcol.sdm.present)

heatmap!(ax2, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(ax2, rotl90(coordinates[FP,:]); markersize = 3, color = bkcol.sdm.absent, marker = :cross)

heatmap!(ax4, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(
    ax4,
    rotl90(coordinates[FN,:]);
    markersize = 3,
    color = bkcol.sdm.present,
    marker = :cross,
)

heatmap!(ax5, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(ax5, rotl90(coordinates[TN,:]); markersize = 2, color = bkcol.sdm.absent)

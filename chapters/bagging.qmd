...
JLD2.jldopen(joinpath(_models_path, "sdm-training-data.jld2"), "r") do file
    ...
    global bags = file["bags"]
end;


# Load the model
f = JLD2.jldopen(modelpath, "r")
model = f["02-thresholded"]
close(f)
...
#| echo: false
#| output: false
heatmap(IQR; colorrange=(0.0, 0.2), colormap=nightfall)
```
...
scatter!(ax1, fpr.(E), tpr.(E), color=vibrant[1], marker=:cross)
scatter!(ax1, (fpr(C), tpr(C)), color=:black)
scatter!(ax1, (fpr(OOB), tpr(OOB)), color=vibrant[2])

scatter!(ax2, tpr.(E), ppv.(E), color=vibrant[1], marker=:cross)
scatter!(ax2, (tpr(C), ppv(C)), color=:black)
scatter!(ax2, (tpr(OOB), ppv(OOB)), color=vibrant[2])

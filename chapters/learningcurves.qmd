...
fig = Figure(; resolution=(500, 300))
ax = Axis(fig[1,1]; xlabel="Threshold", ylabel="Predictive value")



lines!(ax, T, vec(mean(ppv.(CT); dims=1)), color=bright[2], linewidth=2, linestyle=:solid)
lines!(ax, T, vec(mean(npv.(CT); dims=1)), color=bright[3], linewidth=2, linestyle=:dash)
...
T = LinRange(0.0, 1.0, 20)
Pr = LinRange(0.1, 0.9, 20)
for pr in Pr
    model.classifier.prior = pr
    CT = hcat([first(crossvalidate(model, folds; τ=τ, threshold=false)) for τ in T]...)
...
Threads.@threads for k in keys(pr)
    x = [bio[i][k] for i in 1:19]
    pr[k] = predict(model, x; threshold=false)
    en[k] = entropy(pr[k])
end
```
...
heatmap!(ax, pr .>= model.τ, colormap=vibrant[1:2])
ax2 = Axis(f[1,2], aspect=DataAspect())
heatmap!(ax2, en, colormap=iridescent)
for a in [ax, ax2]
    hidedecorations!(a)
    hidespines!(a)
end

...
include(joinpath(_code_path, "minisdm/varimp.jl"))
```
...
#| output: false
reset!(model)
b0 = predictors(backwardselection!(model, folds; threshold=false))
Cb0v, Cb0t = crossvalidate(model, folds; threshold=false)

reset!(model)
f0 = predictors(forwardselection!(model, folds; threshold=false))
Cf0v, Cf0t = crossvalidate(model, folds; threshold=false)

reset!(model)
c0 = predictors(constrainedselection!(model, folds, [1,12]; threshold=false))
Cc0v, Cc0t = crossvalidate(model, folds; threshold=false)

reset!(model)
i0 = predictors(stepwisevif!(model, 50.0; threshold=false))
...
tbl = []
push!(tbl, ["@sec-classification baseline", "1,12", mean(mcc.(C0v))])
push!(tbl, ["All variables", " ", mean(mcc.(CXv))])
push!(tbl, ["Fwd. sel.", join(f0, ","), mean(mcc.(Cf0v))])
push!(tbl, ["Constr. sel.", join(c0, ","), mean(mcc.(Cc0v))])
push!(tbl, ["Backw. sel.", join(b0, ","), mean(mcc.(Cb0v))])
push!(tbl, ["Var. infl. fac.", join(i0, ","), mean(mcc.(Ci0v))])
...
pretty_table(
    D;
    backend = Val(:markdown),
    header = ["Model", "Variables", "MCC"],
    alignment = [:l, :l, :l],
    formatters = ft_printf("%4.3f", 3),
...
current_best_model = SDM(RawData(), NBC(), 0.5, X, y, [1,12])

# Baseline
mod = SDM(RawData(), NBC(), 0.5, X, y, [1,12])
train!(mod; threshold=false)
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["@sec-classification baseline", " ",length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
...
# Baseline + PCA
mod = SDM(MultivariateTransform{PCA}(), NBC(), 0.5, X, y, [1,12])
train!(mod; threshold=false)
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Fwd. sel.", " ",length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Backw. sel.", " ",length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
# Constrained
mod = SDM(RawData(), NBC(), 0.5, X, y, collect(1:19))
constrainedselection!(mod, folds, [1, 12]; threshold=false)
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Constr. sel.", " ",length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
for (i,v) in enumerate(model.v)
    R = variableimportance(model, folds, v; threshold=false, reps=20)
    Vm[i] = mean(R)
end
...
#| output: false
#| echo: false
origmodel = SDM(RawData(), NBC(), 0.5, model.X, model.y, [1,12])

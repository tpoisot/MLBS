...
# Selecting variables {#sec-predictors}
...
# Baseline + Whitening
mod = SDM(MultivariateTransform{Whitening}(), NBC(), 0.5, X, y, [1,12])
train!(mod; threshold=false)
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["", "Whitening", length(predictors(mod)), mean(mcc.(Csv)), mean(mcc.(Cst))])
...
# Forward + Whitening
mod = SDM(MultivariateTransform{Whitening}(), NBC(), 0.5, X, y, collect(1:19))
forwardselection!(mod, folds; threshold=false)
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["", "Whitening", length(predictors(mod)), mean(mcc.(Csv)), mean(mcc.(Cst))])
...
finalmodel = SDM(MultivariateTransform{PCA}(), NBC(), 0.5, X, y, [1,12])
reset!(finalmodel)
constrainedselection!(finalmodel, folds, [1,12]; threshold=false)
...
Threads.@threads for k in keys(opr)
    preds = [bio[i][k] for i in 1:19]
    opr[k] = predict(origmodel, preds)
    vpr[k] = predict(finalmodel, preds)
end
```
...
prd = predict(finalmodel; threshold = true)
grd = y

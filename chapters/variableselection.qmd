...
include(joinpath(_code_path, "minisdm/transformations.jl"))
...
include(joinpath(_code_path, "minisdm/vif.jl"))
include(joinpath(_code_path, "minisdm/variableselection.jl"))
```
...
train!(model, y, X[v0,:]; classify=false)
```

```{julia}
#| echo: false
#| output: false
folds = kfold(y, X[v0,:]; k=15, permute=true)
C0v, C0t = crossvalidate(model, y, X[v0,:], folds)
CXv, CXt = crossvalidate(model, y, X, folds)
```
...
#| echo: false
#| output: false
b0 = backwardselection(model, y, X, folds, mcc)
Cb0v, Cb0t = crossvalidate(model, y, X[b0,:], folds)
f0 = forwardselection(model, y, X, folds, mcc)
Cf0v, Cf0t = crossvalidate(model, y, X[f0,:], folds)
c0 = constrainedselection(model, y, X, folds, [1,12], mcc)
Cc0v, Cc0t = crossvalidate(model, y, X[c0,:], folds)
i0 = stepwisevif(X, 10.0)
Ci0v, Ci0t = crossvalidate(model, y, X[i0,:], folds)
```
...
for (sk, sf) in sel
    for (tk, tf) in trf
        @info "mod"
        mod = SDM(tf(), NBC(), Thresholder())
        @info "varsel"
        vs = sf(mod, y, X, folds, mcc)
        @info size(X)
        Csv, Cst = crossvalidate(mod, y, X[vs,:], folds)
        row = "| $(sk) | $(tk) | $(length(vs)) |"
        for (m, p) in msrs

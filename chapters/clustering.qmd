...
There are two interesting things in @fig-kmeans-tuning. First, note that for $k=4$, there is almost no dispersal: all of the assignments have the exact same score, which is unlikely to happen except if the assignments are the same every time! This is a good sign, and, anecdotally, something that might suggest a really information separation of the points. Second, $k = 4$ has by far the lowest Davies-Bouldin index of all values we tried, and is therefore strongly suggestive of an optimal hyper-parameter.

In this specific case, it makers very little sense *not* to use $k = 4$. We will likely lose a cluster to water pixels, but the rest of the landmass will be split into three categories. In some cases, there will be values of $k$ that are very close to one another in terms of how well they partition the data. In these cases, applying your best judgment is required! There are many cases where it is acceptable to sacrifice a little bit of optimality in order to present more actionable information.

### Clustering with optimal number of classes

The clustering of pixels using $k = 4$ is presented in @fig-kmeans-optimal-landscape. Unsurprisingly, *k*-means separated the open water pixels, what looks like urban and more arid areas, and split vegetation in two classes. This is excellent information! There are two relevant classes for our question of "where are areas with vegetation?", and we can look for reasons why that would be the case.

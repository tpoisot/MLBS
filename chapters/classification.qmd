...
Why not pick one of these instead of the MCC? Well, all modeling is compromise; we don't want a model to become too good at prediciting absences, to the point where prediction about presences would become meaningless. Selecting models on the basis of a measure that only emphasizes one outcome is a risk that we shouldn't be willing to take. For this reason, measures that are good at optimising the value of a negative and a positive predictions are far better representations of the performance of a model. The MCC does just this.
...
The output of cross-validation is given in @fig-classification-crossvalidation (and compared to the no-skill classifier in @tbl-classification-crossvalidation). As we are statisfied with the model performance, we can re-train it using all the data (*but not the part used for testing*) in order to make our first series of predictions.
...
::: column-margin
Take a minute to think about which places are more likely to have lower temperatures on a Island. Is there an additional layer of geospatial information we could add that would be informative?
:::
...
ax0 = GeoAxis(f[1,1], xlabel="Longitude", ylabel = "Latitude", lonlims=extrema(longitudes(pr)), latlims=extrema(latitudes(pr)), dest = "+proj=moll")
ax1 = GeoAxis(f[1,2], xlabel="Longitude", ylabel = "Latitude", lonlims=extrema(longitudes(pr)), latlims=extrema(latitudes(pr)), dest = "+proj=moll")
ax2 = GeoAxis(f[1,3], xlabel="Longitude", ylabel = "Latitude", lonlims=extrema(longitudes(pr)), latlims=extrema(latitudes(pr)), dest = "+proj=moll")

surface!(ax1, pr.>0.5, shading=false, colormap=vibrant[1:2], interpolate=false)
sf = surface!(ax2, un, shading=false, colormap=cgrad(iridescent), interpolate=false)

surface!(ax0, pr, colormap=[:black, :black], shading=false, interpolate=false)

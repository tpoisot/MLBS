...
axtemp = Axis(gl[1,1])
density!(axtemp, 0.1filter(r -> r.presence, occdata).BIO1, color=bkcol.sdm.presentbg, strokecolor=bkcol.sdm.present, strokewidth=1)
density!(axtemp, 0.1filter(r -> !r.presence, occdata).BIO1, color=bkcol.sdm.absentbg, strokecolor=bkcol.sdm.absent, strokewidth=1)

axprec = Axis(gl[2,2])
density!(axprec, filter(r -> r.presence, occdata).BIO12, color=bkcol.sdm.presentbg, strokecolor=bkcol.sdm.present, strokewidth=1, direction=:y)
density!(axprec, filter(r -> !r.presence, occdata).BIO12, color=bkcol.sdm.absentbg, strokecolor=bkcol.sdm.absent, strokewidth=1, direction=:y)
...
scatter!(axboth, xn, yn, color=bkcol.sdm.absent, markersize=4, label="Pseudo-absence")
scatter!(axboth, xp, yp, color=bkcol.sdm.present, markersize=4, label="Presence")
...
ax = Axis(gl[2,1], xlabel="Mean air temperature (°C)", ylabel = "Annual precipitation (kg m⁻²)")
heatmap!(ax, 0.1air, prc, SRF, colormap=[bkcol.sdm.absent, bkcol.sdm.present], alpha=0.3)
xp = 0.1filter(r -> r.presence, occdata).BIO1
xn = 0.1filter(r -> !r.presence, occdata).BIO1
yp = filter(r -> r.presence, occdata).BIO12
yn = filter(r -> !r.presence, occdata).BIO12
scatter!(ax, xp, yp, color=bkcol.sdm.present, markersize=4, label="Presence")
scatter!(ax, xn, yn, color=bkcol.sdm.absent, markersize=4, label="Pseudo-absence")
...
scatter!(ax1, latlon[TP, 2], latlon[TP, 1]; markersize = 2, color = bkcol.sdm.present)

heatmap!(ax2, pr; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(ax2, latlon[FP, 2], latlon[FP, 1]; markersize = 3, color = bkcol.sdm.absent, marker = :cross)
...
scatter!(ax5, latlon[TN, 2], latlon[TN, 1]; markersize = 2, color = bkcol.sdm.absent)

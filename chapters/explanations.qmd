...
idx = 860 # 860 is a good neutral one
S = [shapley(model, X, idx, j; M=1000) for j in eachindex(v)]
...
Threads.@threads for i in axes(X, 1)
    for j in axes(X, 2)
        S[i,j] = shapley(model, X, i, j; M=500)
    end
end
P = [model(X[i,:]) for i in axes(X, 1)]
```
...
fig = Figure()
ax = Axis(fig[1,1])
barplot!(ax, varimp[reverse(vord)], direction=:x, bar_labels = "BIO".*string.(v[reverse(vord)]), flip_labels_at = 0.05)
current_figure()
```
...
```{julia}
f = Figure()
args = (color=y, markersize=5, colorrange=(0., 1.), colormap=vibrant[1:2])

ax1 = Axis(f[1,1]; xlabel="BIO $(v[vord[1]])")
scatter!(ax1, X[:,vord[1]], S[:,vord[1]]; args...)
ylims!(ax1, maximum(abs.(extrema(S[:,vord[1]]))).*(-1,1))
ax2 = Axis(f[2,1]; xlabel="BIO $(v[vord[2]])")
scatter!(ax2, X[:,vord[2]], S[:,vord[2]]; args...)
ylims!(ax2, maximum(abs.(extrema(S[:,vord[2]]))).*(-1,1))
ax3 = Axis(f[1,2]; xlabel="BIO $(v[vord[3]])")
scatter!(ax3, X[:,vord[3]], S[:,vord[3]]; args...)
ylims!(ax3, maximum(abs.(extrema(S[:,vord[3]]))).*(-1,1))
ax4 = Axis(f[2,2]; xlabel="BIO $(v[vord[4]])")
scatter!(ax4, X[:,vord[4]], S[:,vord[4]]; args...)
ylims!(ax4, maximum(abs.(extrema(S[:,vord[4]]))).*(-1,1))
...
Threads.@threads for k in keys(V[1])
    x = [b[k] for b in bio]
    for i in axes(v, 1)
        V[i][k] = shapley(model, X, x, i; M=500)
    end
end
```

...
#| echo: false
#| output: false
model = deepcopy(current_best_model)
```
...
for (i,v) in enumerate(model.v)
    Vm[i] = variableimportance(model, folds, v; threshold=false, samples=20)
end
...
scatter!(ax1, coordinates[TP, 2], coordinates[TP, 1]; markersize = 2, color = bkcol.sdm.present)

heatmap!(ax2, final_range; colormap = [bkcol.sdm.absentbg, bkcol.sdm.presentbg])
scatter!(ax2, coordinates[FP, 2], coordinates[FP, 1]; markersize = 3, color = bkcol.sdm.absent, marker = :cross)
...
scatter!(ax5, coordinates[TN, 2], coordinates[TN, 1]; markersize = 2, color = bkcol.sdm.absent)

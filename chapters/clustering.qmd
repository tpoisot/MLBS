...
| 6, 7   | Short wavelength IR (SWIR 1, 2) | Good at differentiating wet earth and dry earth |
| 8      | Panchromatic                    | High-resolution monochrome                      |
| 9      | Cirrus band                     | Can pick up high and thin clouds                |
| 10, 11 | Thermal infrared                |                                                 |

: Overview of the bands in a Landsat 8 scene. The data from this chapter were downloaded from the Google Earth Engine API.
...
#| echo: false
tbl = ["| Cluster | Cover | NDVI | NDWI | NDMI | NBR |", "|-----|-----|---|-------|------|--|"]
ctr = clusters.centers
for cluster in axes(ctr, 2)
    cover = round(Int, count(clusters.assignments .== cluster)/length(clusters.assignments)*100)
    _ndvi = round(ctr[1,cluster]; digits=2)
    _ndwi = round(ctr[2,cluster]; digits=2)
    _ndmi = round(ctr[3,cluster]; digits=2)
    _nbr = round(ctr[4,cluster]; digits=2)
    push!(tbl,
        "| $(cluster) | $(cover) | $(_ndvi) | $(_ndwi) | $(_ndmi) | $(_nbr) |"
    )
end
    
push!(tbl, "\n: Summary of the values for the centers of the optimal clusters found in this image. The cover column gives the percentage of all pixels associated to this class. {#tbl-clustering-centers}")

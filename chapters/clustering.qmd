...
In fact, we are going to use an image with many dimensions: the data in this chapter are coming from a Landsat 8 scene [@vermote2016], for which we have access to 9 different bands.
...
: Overview of the bands in a Landsat 8 scene. The data from this chapter were downloaded from [LandsatLook](https://landsatlook.usgs.gov).
...
hexbin!(ax, NDVI, NDMI, colormap=cgrad(incandescent), bins=20)
tightlimits!(ax)
current_figure()
```

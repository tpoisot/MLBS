...
push!(tbl, ["Fwd. sel.", " ", mean(mcc.(Cf0v))])
...
trf = ["Raw data" => RawData, "PCA" => MultivariateTransform{PCA}, "Whitening" => MultivariateTransform{Whitening}]
...
for (sk, sf) in sel
    for (tk, tf) in trf
        mod = SDM(tf(), NBC(), 0.5, X, y, [1,12])
        reset!(mod)
        sf(mod, folds; threshold=false)
        vs = predictors(mod)
...
train!(finalmodel; threshold=false)

...
| Parameter | Step        | Default value | Constraints                     |
|-----------|-------------|---------------|---------------------------------|
| $\alpha$  | reflection  | $1$           | $\alpha > 0$                    |
| $\beta$   | contraction | $1/2$         | $0 < \beta < 1$                 |
| $\gamma$  | expansion   | $2$           | $\gamma > 1$, $\gamma > \alpha$ |
| $\delta$  | shrinkage   | $1/2$         | $0 < \delta < 1$                |

```{julia}
f = Figure()
ax = Axis(f[1,1])
scatter!(ax, tuple(x[model.v]...), marker=:diamond, color=bkcol.generic, markersize=12)
for _ in 1:80
    xc = counterfactual(model, x, 0.35, 50.0; threshold=false)

...
In the previous section, we observed that variable selection could in fact make the model a little better, 
...
#| echo: false
#| fig-cap: Consequences of different variable transformations on the predicted range of *Sitta whiteheadi*, as introduced in @fig-classification-range. The left panel shows the effect of moving from two predictors to the best set of variables identified with forward selection. The right panel then shows the effect of adding a data transformation step (Whitening) *and* a variable selection step, compared to variable selection alone. Note that the small area of predicted presence in the Capicorsu (the Northern tip) has disappeared with a better variable transformation.
final_range = vpr .> 0.5
inter_range = ipr .> 0.5
old_range = opr .> 0.5
rangediff(new, old) = mask(new .| old, new .- old)
fig = Figure()
ax1 = Axis(fig[1,1,]; aspect=DataAspect(), title="Selection")
ax2 = Axis(fig[1,2,]; aspect=DataAspect(), title="Transformation")
heatmap!(ax1, final_range, colormap=[:lightgrey, :lightgrey], alpha=0.7)
heatmap!(ax2, final_range, colormap=[:lightgrey, :lightgrey], alpha=0.7)
hm = heatmap!(ax1, rangediff(inter_range, old_range); colormap = Reverse(cgrad(nightfall, 3, categorical=true)))
heatmap!(ax2, rangediff(final_range, inter_range); colormap = Reverse(cgrad(nightfall, 3, categorical=true)))
Colorbar(fig[1,3], hm; tellheight=false, height=Relative(0.4))
current_figure()
```

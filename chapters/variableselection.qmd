...
push!(tbl, ["@sec-classification baseline", "none", length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Fwd. sel.", "none", length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Backw. sel.", "none", length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
vs = predictors(mod)
Csv, Cst = crossvalidate(mod, folds; threshold=false)
push!(tbl, ["Constr. sel.", "none", length(predictors(mod)), "", "", mean(mcc.(Csv)), mean(mcc.(Cst))])
if mean(mcc.(Csv)) > current_best_mcc
    current_best_mcc = mean(mcc.(Csv))
    current_best_model = deepcopy(mod)
end
...
ax = Axis(f[1,1], xticks = (1:length(Vm), ["BIO$(v)" for v in model.v][sortperm(Vm)]))

stem!(ax, Vm[sortperm(Vm)] ./ sum(Vm), strokecolor=bkcol.generic, color=:white, strokewidth=1, trunkcolor=:transparent, stemcolor=bkcol.nodata, markersize=18)

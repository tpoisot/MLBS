...
ldesc = JSON.parsefile("data/occurrences/layers.json")
...
emphasize that the model is two functions, a transformer and the classifier

yes even when not trasnforming, this is $f(x) = x$ which is a function

## A digression: what is a variable?
...
**ARGUMENT** do at the global scale – but then question: is the structure of data outside of the study area relevant? This way to introduce data leakage is extremely common in the species distribution literature [see e.g. @demarco2018]. suggest reasons for/against
...
#| tbl-cap: Model performance when coupling variable selection with variable transformation. The measures of performance are given as in @tbl-predictors-selection, and as we use the same folds for validation, can be directly compared.
tbl = []
...
for (i,v) in enumerate(model.v)
    R = variableimportance(model, folds, v; threshold=false, reps=50)
    Vm[i] = mean(R)
end
...
show the difference of two predictors v. two PCs

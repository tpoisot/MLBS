...
JLD2.jldsave(trainingpath; y=y, X=X, latlon=latlon)
```
...
F = permutedims(hcat(values.(bio)...))
pr.grid[findall(!isnothing, bio[1].grid)] .= predict(model, F; threshold=true)
```
...
scatter!(ax2, latlon[TP,2], latlon[TP,1], markersize=5, color=vibrant[2])
scatter!(ax2, latlon[FP,2], latlon[FP,1], markersize=6, marker=:cross, color=:black)

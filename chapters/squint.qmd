...
```{julia}
threshold = LinRange(0.0, 0.5, 100)
pr = Float64[]
sp = Float64[]
for t in threshold
    model.τ = t
    p = predict(model)
    push!(pr, mean(p))
    push!(sp, tpr(ConfusionMatrix(p, model.y)))
end
```
...
```{julia}
S = explain(model, 12; threshold=true)
scatter(model.X[12,:], S, colormap=[bkcol.sdm.absent, bkcol.sdm.present], color=model.y)
```
...
allvar = deepcopy(model)
allvar.v = collect(1:19)
CXv, CXt = crossvalidate(allvar, folds)
```
...
reset!(model)
constrainedselection!(model, folds, [1,12]; threshold=false)
c0 = predictors(model)
Cc0v, Cc0t = crossvalidate(model, folds; threshold=false)

reset!(model)
stepwisevif!(model, 10.0; threshold=false)
i0 = predictors(model)

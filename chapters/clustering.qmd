...
In fact, we are going to use an image with many dimensions: the data in this chapter are coming from a Landsat 9 scene [@vermote2016], for which we have access to 7 different bands (the full data product has more bands, but we will not use them all in this example).
...
: Overview of the bands in a Landsat 9 scene. The data were downloaded from [LandsatLook](https://landsatlook.usgs.gov).
...
Because there are a few guidelines (educated guesses, in truth, and the jury is still out on the "educated" part) about the values we are using in this section, we can look at the relationship between the NDVI and NDMI data [@fig-kmeans-hexbin]. For example, NDMI values around -0.1 are [low-canopy cover with low water stress](https://eos.com/make-an-analysis/ndmi/); NDVI values from 0.2 to 0.5 are good candidates for moderately dense crops.
...
In this specific case, we will sacrifice a little bit of optimal clustering for a little more classes, and use $k = 4$. Is it the optimal partition of our pixels? No. But the way to read @fig-kmeans-tuning is not necessarily "pick $k=3$ or else". Instead, it is closer to "whatever you do, do not pick $k=6$". Just because we start using measures of model performance does not mean we can stop using critical thinking.

...
now we run vif with a threshold of 10

```{julia}
vifvars = stepwisevif(trainfeatures, 10.0)
C = [last(confusion_for_fold(trainlabels, trainfeatures, fold; vars=vifvars)) for fold in folds]

variableset = copy(vifvars)
...
_mcc = mean(mcc.(C))
push!(varsel, 
    (Method = :VIF, N = length(variableset), Accuracy = _acc, TSS = _tss, NPV = _npv, PPV = _ppv, MCC = _mcc)
)

@info variableset
```

and now we run a PCA -- trained on the training data, measured on the validation data, k-fold cross validation

```{julia}
using MultivariateStats

C = zeros(ConfusionMatrix, length(folds))
v = zeros(Int64, length(C))
for (i,fold) in enumerate(folds)
    pX = fit(PCA, permutedims(trainfeatures[fold[1],:]))
    v[i] = outdim(pX)
    vX = permutedims(predict(pX, permutedims(trainfeatures[fold[2],:])))
    tX = permutedims(predict(pX, permutedims(trainfeatures[fold[1],:])))
    
    predictor = naivebayes(trainlabels[fold[1]], tX)
    
    prediction = vec(mapslices(predictor, tX, dims=2))
    validation = vec(mapslices(predictor, vX, dims=2))

    C[i] = ConfusionMatrix(validation, trainlabels[fold[2]])
end
...
_mcc = mean(mcc.(C))
push!(varsel, 
    (Method = :PCA, N = mean(v), Accuracy = _acc, TSS = _tss, NPV = _npv, PPV = _ppv, MCC = _mcc)
)
```
...
for col in [:Accuracy, :TSS, :NPV, :PPV, :MCC]
    varsel[!,col] = round.(varsel[!,col]; digits=2)
end
sort!(varsel, :MCC)
```

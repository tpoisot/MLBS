...
b0 = predictors(model)
Cb0v, Cb0t = crossvalidate(model, folds)

reset!(model)
forwardselection!(model, folds)
f0 = predictors(model)
Cf0v, Cf0t = crossvalidate(model, folds)

reset!(model)
constrainedselection!(model, folds, [1,12])
c0 = predictors(model)
Cc0v, Cc0t = crossvalidate(model, folds)

reset!(model)
stepwisevif!(model, 10.0)
i0 = predictors(model)
Ci0v, Ci0t = crossvalidate(model, folds)
```
...
for (sk, sf) in sel
    for (tk, tf) in trf
        ...
        vs = predictors(mod)
        @info size(X)
        Csv, Cst = crossvalidate(mod, folds)
        row = "| $(sk) | $(tk) | $(length(vs)) |"
        for (m, p) in msrs

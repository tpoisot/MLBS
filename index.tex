...
in \textbf{?@sec-variable-selection}.
...
\sidecaption{\label{fig-kmeans-initial-landscape}caption}
...
\sidecaption{\label{fig-crossvalidation-overview}Overview of the
...
\sidecaption{\label{fig-splits-prediction}Overview of the fit of the
final model (trained on all the training examples), visualized as the
time series. Note that the year was not used as a variable in the model.
The purple part of the prediction corresponds to the prediction of the
model for the testing data, which are zoomed-in on in
...
\sidecaption{\label{fig-splits-detail}Overview of the model predictions
on the testing data. Note that the model still smoothes out some of the
...
\includegraphics{chapters/classification_files/figure-pdf/fig-classification-separability-output-1.png}

}

\sidecaption{\label{fig-classification-separability}This figures show
the separability of the presences (orange) and pseudo-absences (grey) on
the temperature and precipitation dimensions.}
...
picks at random (\emph{i.e.} with probability \(1/2\)) within these
classes.
...
\includegraphics{chapters/classification_files/figure-pdf/fig-classification-crossvalidation-output-1.png}

}

\sidecaption{\label{fig-classification-crossvalidation}Overview of the
...
\includegraphics{chapters/classification_files/figure-pdf/fig-classification-decision-output-1.png}

}

\sidecaption{\label{fig-classification-decision}Overview of the decision
boundary between the positive (blue) and negative (classes) using the
NBC with two variables. Note that, as expected with a Gaussian
...
\includegraphics{chapters/classification_files/figure-pdf/fig-classification-range-output-1.png}

}

\sidecaption{\label{fig-classification-range}Occurence data (left),
prediction of presences in space (middle), with the uncertainty in the
prediction derived from bootstrap replicates (right). As we could have
anticipated from the high values of the MCC, even this simple model does
an adequate job at predicting the presence of \emph{Sitta whiteheadi}.}
...
\sidecaption{\label{fig-predictors-leakage}Overview of a data
transformation pipeline that introduces data leakage (left), or that
does not introduce data leakage (right). In both cases, a transformation
such as a PCA is applied to the data; in the example on the right, it is
applied as part of the model, and can therefore be applied without
breaking the train/predict separation. The pipeline on the left
introduces data leakage, as the training data will be changed by
information contained in the validation data.}
...
\sidecaption{\label{fig-predictors-workflow}Overview of the variable
...
\includegraphics{chapters/variableselection_files/figure-pdf/fig-predictors-rangediff-output-1.png}

}

\sidecaption{\label{fig-predictors-rangediff}Consequences of different
...
\emph{need} to be a probability; it simply needs to be on an interval or
ratio scale.
...
\includegraphics{chapters/learningcurves_files/figure-pdf/fig-tuning-threshold-output-1.png}

}

\sidecaption{\label{fig-tuning-threshold}learning curve for the
threshold}
...
\includegraphics{chapters/learningcurves_files/figure-pdf/fig-tuning-roc-pr-output-1.png}

}

\sidecaption{\label{fig-tuning-roc-pr}ROC and PR}
...
\includegraphics{chapters/learningcurves_files/figure-pdf/fig-tuning-both-output-1.png}

}

\sidecaption{\label{fig-tuning-both}learning curve for the threshold and
prior}

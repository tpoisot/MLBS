...
hexbin!(ax, NBR, NDWI, colormap=cgrad(iridescent), bins=50)
tightlimits!(ax)
...
heatmap!(ax, C; colormap=medium[1:3])
hidedecorations!(ax)
current_figure()
```
...
::: column-margin
Note that using $k=3$ on the non-water pixel is *different* from using $k=4$ on all pixels! This is an important distinction, as it accentuates the importance of data selection and preparation on the process of picking good hyper-parameters.
:::
...
heatmap!(ax_clust, permutedims(C); colormap=medium[2:(k+1)])
...
scatter!(ax, values(mask(msk, NDVI)), values(mask(msk, NDMI)), color=clusters.assignments, colormap=medium[2:(k+1)], markersize=2)
tightlimits!(ax)
current_figure()
```

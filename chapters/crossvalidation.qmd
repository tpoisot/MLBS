...
using GLM
include(joinpath(_code_path, "minisdm", "crossvalidation.jl"))
```
...
![Overview of the cross-validation process, illustrating two splits using the k-folds strategy (@sec-crossvalidation-kfolds). The testing dataset is not used until we are ready to evaluate the predicted performance of the model after we have trained the best possible model.]
...
#| echo: false
#| fig-scap: Relationship between temperature in March and day of blooming.
...
#| echo: false
#| fig-scap: Illustration of different folds on a timeseries.
...
for (i,s) in enumerate([s1, s2, s3, s4])
    ...
    scatter!(s, sakuratest.year, sakuratest.flowavg, color=Makie.wong_colors()[4])
    j = 2i-1
    ...
    scatter!(s, valid.year, valid.flowavg, color=Makie.wong_colors()[3])
    hidedecorations!(s)
    hidespines!(s)
end
current_figure()
...
for (i, Ki) in enumerate(folds)
    if Ki < size(sakuratrain, 1)
        ...
        valid = sakuratrain[valid_idx, :]
        train = sakuratrain[train_idx, :]
        m1 = GLM.lm(@formula(flowavg ~ temperature), train)
...
#| echo: false
#| output: false
model = GLM.lm(@formula(flowavg ~ temperature), sakuratrain)
...
#| fig-scap: Model performance according to three loss functions.
...
#| echo: false
#| fig-scap: Fit of the final model, after training on all instances.
...
lines!(ax, sakuratrain.year, convert(Vector{Float64}, predict(model, sakuratrain)), color=:black, linewidth=1)
lines!(ax, sakuratest.year, convert(Vector{Float64}, predict(model, sakuratest)), color=Makie.wong_colors()[4], linewidth=2)
current_figure()
```
...
#| echo: false
#| fig-scap: Model predictions on the testing data.
...
lines!(ax, sakuratest.year, convert(Vector{Float64}, predict(model, sakuratest)), color=:red)
current_figure()
```
